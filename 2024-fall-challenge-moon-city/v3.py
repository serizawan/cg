...
class City:
    ...
    def build_tube_network(self, actions):
        ...
        if not self.network.tubes:
            ...
            self.extend_tube_network(from_building, actions)
    ...
    def extend_tube_network(self, from_building, actions):
        ...
        while not is_to_added and i < len(closest_buildings):
            ...
            if not next_closest_building.tos and self.is_allowed(tube) and self.n_resources >= tube.cost:
                ...
                is_to_added = True
                self.extend_tube_network(next_closest_building, actions)
            i += 1
...
class Building(Item):
    ...
    def find_itinerary_to_closest_typed_module(self, module_type, enable_tp, max_depth=14):
        queue = deque()
        queue.append((self, 0))
        visited = set()
        visited.add(self)
